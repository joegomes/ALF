...
def identity_build(start_system, molecule_library_dir):
    ...
    if type(start_system) == list:
        for n in range(len(start_system)):
            start_system[n][1] = read(xyz[n])
    else:
        start_system[1] = read(xyz)
...
def identity_metad(start_system, ase_calculator, xtb_command='xtb', hmass=2, time=50., temp=400., step=0.5, shake=0,
...
def identity_build_task(moleculeids, builder_config):
    """
    ...
    system = identity_build(empty_systems, **builder_config)
    if type(system) == list:
        for s in system:
            system_checker(s)
    else:
...
def identity_metad_task(molecule_object, sampler_config, model_path, current_model_id, gpus_per_node):
    """
